...
def create_deck_image(deck_list_dict, magic_cards, deck_id=0, output_file='deck_image.png'):
    '''Lager en collage av kort i stokken, sortert etter type og mana value.'''
    
    deck = deck_list_dict[deck_id]['main']  # Vi bruker kun main deck
    categorized_cards = { 'land': [], 0: [], 1: [], 2: [], 3: [], 4: [], 5: [], 6: [], 7: [] }

    for card_name in deck:
        card_info = magic_cards.get(card_name, None)
        if not card_info:
            continue  # hopp over ukjente kort
    ...
    # Hent kortbilder og organiser dem i kolonner
    ...
    outer_padding = 40  # Ny ramme rundt bildet

    num_columns = len(columns)
    max_rows = max(len(col) for col in columns)

    # Justert total størrelse for ramme
    ...
    #print(f'Deck image saved as {output_file}')

    
def sanitize_filename(card_name):
    '''Fjerner problematiske tegn og tar kun første navn ved //.'''
...
def fetch_card_image(card_name, folder='program/card_images'):
    '''Henter kortbilde fra lokal mappe om mulig, ellers fra Scryfall og lagrer lokalt.'''
    ...
    if os.path.exists(local_path):
        try:
            ...
            print(f"Feil ved åpning av {local_path}, forsøker å hente nytt bilde. ({e})")

    # Hvis ikke lokalt, hent fra Scryfall
    try:
...
def resize_image(image, width=CARD_WIDTH, height=CARD_HEIGHT):
    '''Skalerer bildet til fast størrelse med proporsjonal tilpasning og hvit bakgrunn.'''
...
def make_deck_images(decklist_folder, deck_dict, magic_cards, save_folder):
    ...
    for deck_id, deck_file in zip(deck_ids, deck_files):
        ...
        try:
            ...
            print(f'Feil ved bilde for {deck_file}: {e}')
